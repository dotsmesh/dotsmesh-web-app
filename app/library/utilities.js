...
(x) => {
    ...
    x.icons = {
        ...
        'push-notifications': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" color="#fff" viewBox="0 0 24 24"><path d="M18 16v5a1 1 0 01-1 1H7a1 1 0 01-1-1V3a1 1 0 011-1h10a1 1 0 011 1v.5M11 19h2"/><path stroke-width="2" d="M20.35 8.84v1.96c0 .78.26 1.3.78 1.56h-6.27c.53-.26.79-.78.79-1.56V8.84h0A2.35 2.35 0 0118 6.49h0a2.35 2.35 0 012.35 2.35zm-3.13 3.53a.78.78 0 101.56 0"/></svg>',
        'key': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" color="#fff" viewBox="0 0 24 24"><path d="M21 16v-4h-9"/><circle cx="7" cy="12" r="4"/><path d="M17 15v-3"/></svg>',
        'search': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" color="#fff" viewBox="0 0 24 24"><path d="M14.41 14.41L20 20"/><circle cx="10" cy="10" r="6"/></svg>',
        'globe': '<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" color="#fff" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 22c2.67-2.42 4-5.76 4-10s-1.33-7.58-4-10C9.33 4.42 8 7.76 8 12s1.33 7.58 4 10zM2.5 9h19m-19 6h19"/></svg>',
        'group': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24"><g fill="#fff" stroke="none" transform="translate(-.098 .776)"><circle cx="6.056" cy="5.182" r="2.013"/><circle cx="12.098" cy="5.182" r="2.013"/><circle cx="18.14" cy="5.182" r="2.013"/><circle cx="18.14" cy="11.224" r="2.013"/><circle cx="12.098" cy="11.224" r="2.013"/><circle cx="6.056" cy="11.224" r="2.013"/><circle cx="6.056" cy="17.266" r="2.013"/><circle cx="12.098" cy="17.266" r="2.013"/><circle cx="18.14" cy="17.266" r="2.013"/></g></svg>'
    };

    x.getIconDataURI = (name, color, size = null) => {
        var result = x.icons[name];
        if (color !== undefined) {
            result = x.stringReplaceAll(result, '#fff', color);
        }
        if (size !== null) {
            var matches = result.match(/stroke-width="[0-9\.]*"/g);
            if (matches !== null) {
                for (var match of matches) {
                    var value = parseFloat(match.substr(14, match.length - 15)) * 20 / size;
                    result = result.replace(match, 'stroke-width="' + value + '"');
                }
            }
        }
